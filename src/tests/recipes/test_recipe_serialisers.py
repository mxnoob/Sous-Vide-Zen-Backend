...
class TestRecipeSerializers:
    ...
    def test_create_recipe_serializer(
    ...
    def test_update_recipe_serializer(self, api_client, new_author, new_recipe):
        ...
        response = api_client.patch(
            f"{patch_url}", data=dict(title=example_data.get("title")),
            format="json"
            )
        ...
        response_tags_set = {(tag["name"], tag["slug"]) for tag in
                             response.data["tag"]}
        assert (
                response_tags_set == expected_tags
        ), "Теги в ответе не соответствуют ожидаемым."

    def test_recipe_update_once_per_day(
